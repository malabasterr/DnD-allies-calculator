...
import BruceStats from '../assets/BruceStats.jpg';
...
function Bruce() {
  ...
  const generateHits = () => {
    ...
    for (let i = 0; i < 5; i++) {
      generatedNumbers.push(Math.floor(Math.random() * 20) + 8);
    }
  ...
  const generateNormalDamage = (num) => {
    const generatedNumbers = [];
    for (let i = 0; i < num; i++) {
      generatedNumbers.push((Math.floor(Math.random() * 6) + 1) + (Math.floor(Math.random() * 6) + 5));
    }
  ...
  const generateCritDamage = (num) => {
    const generatedNumbers = [];
    for (let i = 0; i < num; i++) {
      generatedNumbers.push((Math.floor(Math.random() * 6) + 1) + (Math.floor(Math.random() * 6) + 1) + (Math.floor(Math.random() * 6) + 1) + (Math.floor(Math.random() * 6) + 5));
    }
      ...
      <Col className="text-center" xs={12} sm={10} md={8} lg={8} xl={10} xxl={10}>
        ...
        <Col></Col>
          <Col className="text-center" xs={7} sm={7} md={7} lg={7} xl={7} xxl={7}>
            <h1 className="Title fw-bold fs-3">Bruce Almighty</h1>
          </Col>
        ...
        <Row>
          <Col className="text-center">
            <p className="SmallText">Note: Bruce makes two melee attacks, or can use his current stored spell</p>
          </Col>
        </Row>
        <Row xs={1} sm={1} md={3}>
        <Col md={2} lg={2}></Col>
          <Col md={8} lg={8} className="Stats text-center">
            <Image src={BruceStats} fluid/>
          </Col>
          <Col md={2} lg={2}></Col>
        </Row>
        <Row className="align-items-center">
              <Col></Col>
              <Col className="text-center" xs={7} sm={7} md={7} lg={7} xl={7} xxl={7}>
                <Button className="AttackButton" size="sm" variant="outline-dark" onClick={generateHits}>Fist attack</Button>
              </Col>
              <Col></Col>
        </Row>
        <Row>
          ...
          <Col>
            ...
            <Row>
              <Col className="text-left">
                <ol>
                  {toHit.map((number) => (
                    <li className="fw-normal fs-6">{number}{number === 27 ? ' Critical Hit' : ''}{number === 8 ? ' Natural 1' : ''}</li>
                  ))}
                </ol>
              </Col>
            </Row>
