...
import GuardStats from '../assets/GuardStats.jpg';
...
function LordsAlliance() {
  ...
  const generateNormalDamage = (num) => {
    const generatedNumbers = [];
    for (let i = 0; i < num; i++) {
      generatedNumbers.push(Math.floor(Math.random() * 10) + 3);
    }
  ...
  const generateCritDamage = (num) => {
    const generatedNumbers = [];
    for (let i = 0; i < num; i++) {
      generatedNumbers.push((Math.floor(Math.random() * 10) + 3) + (Math.floor(Math.random() * 10) + 1));
    }
